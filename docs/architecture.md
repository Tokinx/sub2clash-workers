...
- 订阅输出同时使用 Cloudflare 边缘缓存（HTTP `Cache-Control`）与 KV 两层：
  ...
  - 缓存层由 Workers Caching 承载（`cache.enabled` + `exports` 双入口）：default 入口关闭缓存作为 gateway，订阅输出转发到 `SubscriptionEntrypoint`（缓存开启），命中缓存时整个入口不执行；purge 按入口作用域隔离，管理台变更经 `purgeByTags` RPC 精确清除。该 RPC 依赖 `ctx.exports` 回环绑定（`compatibility_date` 在 2025-11-17 之后默认开启），路由层经 `c.executionCtx` 取得真实 ExecutionContext 后调用
